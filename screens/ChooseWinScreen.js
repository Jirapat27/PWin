...
const ChooseWinScreen = ({ navigation, route }) => {
  const { destinationCoor } = route.params; // 2. Accessing destinationCoor from route.params
  const [currentLocation, setCurrentLocation] = useState(null);
  const [markers, setMarkers] = useState([]);
  const [selectedMarker, setSelectedMarker] = useState(null);
  const [searchLocation, setSearchLocation] = useState(null);
  const [centerCoordinates, setCenterCoordinates] = useState(destinationCoor); // 3. Initialized centerCoordinates with destinationCoor
  const [winCoor, setWinCoor] = useState(null);
  ...
  const handleMarkerPress = (marker) => {
    ...
    // Store marker's latitude and longitude in winCoor variable
    const { latitude, longitude } = marker;
    setWinCoor({ latitude, longitude });
    console.log(destinationCoor);
  };
  
  useEffect(() => {
    console.log(winCoor); // This will log the updated value of winCoor
  }, [winCoor]); // Run this effect whenever winCoor changes
  
  const confirmDestination = () => {
    if (selectedMarker) {
      navigation.navigate('CalPrice', {
        winData: {
          ...selectedMarker, // Pass all properties of selectedMarker
          winCoor, // Pass winCoor
        },
        destinationCoor,
      });
    }
  ...
  return (
    ...
    <TouchableOpacity style={styles.backButton} onPress={() => navigation.goBack()}>
      ...
      <MapView
        ...
        style={{ flex: 1 }}
        initialRegion={{
          latitude: destinationCoor ? destinationCoor.latitude : 13.0, // 6. Adjusted initial region
          longitude: destinationCoor ? destinationCoor.longitude : 101.0, // 6. Adjusted initial region
          latitudeDelta: 0.0128,
          longitudeDelta: 0.0128,
        }}
      ...
      <View style={styles.head}>
        <Text style={styles.title}>เลือกจุดวิน</Text>
      </View>
      <View style={styles.searchBarContainer}>
        <GooglePlacesAutocomplete
          ref={inputRef}
          placeholder="ค้นหาพื้นที่ของจุดวิน"
          fetchDetails={true}
      ...
      <View style={styles.buttomContainer}>
        <TouchableOpacity style={styles.confirmButton.container} onPress={confirmDestination}>
          <Text style={styles.confirmButton.text}>ยืนยันจุดวิน</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
export default ChooseWinScreen;
